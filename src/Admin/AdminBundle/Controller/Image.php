...
class Image {
    ...
    function thumbnail_full($width, $height) {
        ...
        $this->best_fit($width, $height);
        $x = ($width-$this->width)/2;
        $y = ($height-$this->height)/2;		
        
        imagecopy($new, $this->image, $x, $y, 0, 0, $this->width, $this->height);
	...
	function thumbnail_new($width, $height) {
		...
		if ($old_ratio>=$new_ratio) {
			$rgb = imagecolorat($this->image, $this->width/2, 1);
			$r = ($rgb >> 16) & 0xFF;
			$g = ($rgb >> 8) & 0xFF;
			$b = $rgb & 0xFF;
			$background = imagecolorallocate($new, $r, $g, $b);
			imagefilledrectangle($new, 0, 0, $width, $height/2, $background);
			
			$rgb = imagecolorat($this->image, $this->width/2, $this->height-1);
			$r = ($rgb >> 16) & 0xFF;
			$g = ($rgb >> 8) & 0xFF;
			$b = $rgb & 0xFF;
			$background = imagecolorallocate($new, $r, $g, $b);
			imagefilledrectangle($new, 0, $height/2, $width, $height, $background);
		} else {			
			$rgb = imagecolorat($this->image, 1, $this->height/2);
			$r = ($rgb >> 16) & 0xFF;
			$g = ($rgb >> 8) & 0xFF;
			$b = $rgb & 0xFF;
			$background = imagecolorallocate($new, $r, $g, $b);
			imagefilledrectangle($new, 0, 0, $width/2, $height, $background);
			
			$rgb = imagecolorat($this->image, $this->width-1, $this->height/2);
			$r = ($rgb >> 16) & 0xFF;
			$g = ($rgb >> 8) & 0xFF;
			$b = $rgb & 0xFF;
			$background = imagecolorallocate($new, $r, $g, $b);
			imagefilledrectangle($new, $width/2, 0, $width, $height, $background);
		}
